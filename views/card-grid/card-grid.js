...
export class CardGrid {
  ...
  async load() {
    ...
    try {
      ...
      container.innerHTML = '<p class="p-8 text-center text-bip-red-600">Failed to load content area.</p>';
    }
  }
      ...
      gridContainer.innerHTML = `
        <div class="col-span-full flex flex-col items-center justify-center py-12 px-4">
          <div class="text-center max-w-md">
            <div class="w-16 h-16 mx-auto mb-4 bg-bip-red-100 rounded-full flex items-center justify-center">
              <svg class="w-8 h-8 text-bip-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button data-action="retry" class="px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors duration-200">
              Try Again
            </button>
          </div>
        </div>
    ...
    const cardsHTML = this.cards.map((card, i) => {
      ...
      return `
        <div class="w-full max-w-[560px] flex flex-col card-fade-in" style="animation-delay: ${delay}ms">
          ...
          <div class="px-1">
            <p class="font-normal text-text-primary text-xs sm:text-sm leading-tight sm:leading-[16.8px] text-left transition-colors duration-200 line-clamp-2 overflow-hidden">
